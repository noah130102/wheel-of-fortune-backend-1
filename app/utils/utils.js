...
commonFunctions.hashPassword = (payloadString) =>
  BCRYPT.hashSync(payloadString, CONSTANTS.SECURITY.BCRYPT_SALT);
...
commonFunctions.compareHash = (payloadPassword, userPassword) =>
  BCRYPT.compareSync(payloadPassword, userPassword);
...
commonFunctions.encryptJwt = (payload, expTime = "365d") =>
  JWT.sign(
    payload,
    CONSTANTS.SECURITY.JWT_SIGN_KEY,
    { algorithm: "HS256" },
    { expTime: expTime }
  );
...
commonFunctions.decryptJwt = (token) =>
  JWT.verify(token, CONSTANTS.SECURITY.JWT_SIGN_KEY, { algorithm: "HS256" });
...
commonFunctions.mailSender = async (email, title, body) => {
  try {
    let transporter = nodemailer.createTransport({
      host: CONSTANTS.SMTP_CREDENTIALS.HOST,
      port: CONSTANTS.SMTP_CREDENTIALS.PORT,
      secure: false,
      auth: {
        user: CONSTANTS.SMTP_CREDENTIALS.USER,
        pass: CONSTANTS.SMTP_CREDENTIALS.PASS,
      },
    });
    // Send emails to users
    let info = await transporter.sendMail({
      from: CONSTANTS.SMTP_CREDENTIALS.USER_FROM,
      to: email,
      subject: title,
      html: body,
    });
...
commonFunctions.sendEmail = async (userData, type) => {
  ...
  return await commonFunctions.sendEmailViaAWS(
    userData,
    mailData.Subject,
    result
  );
...
commonFunctions.createResetPasswordLink = (userData) => {
  ...
  let resetPasswordLink =
    CONFIG.WEB_ADMIN_URL +
    "/auth/reset-password/" +
    commonFunctions.encryptJwt(dataForJWT, "1h");
...
commonFunctions.generateExpiryTime = (seconds) =>
  new Date(new Date().setSeconds(new Date().getSeconds() + seconds));
...
commonFunctions.generatePassword = () => {
  ...
  for (var i = 0; i < numLc; ++i) {
    pass.push(commonFunctions.getRandom(lowerCaseLetter));
  }
  for (var i = 0; i < numUc; ++i) {
    pass.push(commonFunctions.getRandom(uperCaseLetter));
  }
  for (var i = 0; i < numDigits; ++i) {
    pass.push(commonFunctions.getRandom(numbers));
  }
  for (var i = 0; i < numSpecial; ++i) {
    pass.push(commonFunctions.getRandom(special));
  }
